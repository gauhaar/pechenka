#!/usr/bin/env python3
"""
Script to fix English text in non-English translation files for the services page.
"""
...
class TranslationFixer:
    ...
    def fix_translations_for_language(self, lang_code: str):
        """Fix translations for a specific language."""
        if lang_code == "en":
            print(f"Skipping English (source language)")
            return
        ...
        print(f"\nFixing translations for {lang_code.upper()}:")
        ...
        for key_path, lang_translations in self.translations.items():
            if lang_code in lang_translations:
                ...
                print(f"  ✓ Fixed {key_path}")
                changes_made += 1
        
        if changes_made > 0:
            # Save the updated file
            self.save_translation_file(lang_file, lang_data)
            print(f"  Total changes: {changes_made}")
        else:
            print(f"  No fixes needed for {lang_code.upper()}")
    ...
    def fix_all_languages(self):
        ...
        for lang_code in sorted(languages_to_fix):
            self.fix_translations_for_language(lang_code)
        
        print(f"\n✅ Fixed translations for {len(languages_to_fix)} languages")
    ...
    def verify_fixes(self):
        ...
        for lang_code in sorted(self.translations.get("pricing.subtitle", {}).keys()):
            lang_file = self.locales_dir / f"{lang_code}.json"
            if not lang_file.exists():
                continue
            ...
            # Check pricing.subtitle
            keys = "pricing.subtitle".split('.')
            current = lang_data
            for key in keys:
            ...
            if current and "Mobile-first plans" not in current:
                print(f"  ✓ {lang_code.upper()}: pricing.subtitle is translated")
            elif current:
                print(f"  ✗ {lang_code.upper()}: pricing.subtitle still has English text")
            else:
                print(f"  ? {lang_code.upper()}: pricing.subtitle not found")
...
def main():
    ...
    print("=" * 60)
    print("This script will fix English text in non-English translation files.")
    ...
    print("\n✅ Done! You can run the detection script again to verify:")
