            ...
            {currentLang === 'en' ? (
              ...
              <>
                <section id="general">
                  ...
                  <p className="mb-4"><strong>1. Предмет:</strong> Регулирование доступа к Silence Environment (Ollama + LLM, SLNC-Code, SitHub).</p>
                  <p className="mb-4"><strong>2. Стороны:</strong> Silence AI (Провайдер) и Пользователь.</p>
                  <p className="mb-4"><strong>3. Акцепт:</strong> Использование/Установка/Подписка означает согласие.</p>
                </section>

                <section id="access">
                  <h2 className="text-2xl font-semibold mb-4">2. Доступ и лицензия</h2>
                  <p className="mb-4"><strong>1. Лицензия:</strong> Ограниченная, неисключительная, непередаваемая лицензия на годовую подписку для локальной инфраструктуры.</p>
                  <p className="mb-4"><strong>2. Ограничения:</strong> Использование только по назначению. Запрещено извлечение/передача кода.</p>
                  <p className="mb-4"><strong>3. Верификация:</strong> Периодические проверки лицензии через хеш/метаданные (максимум раз в 24ч).</p>
                </section>

                <section id="confidentiality">
                  <h2 className="text-2xl font-semibold mb-4">3. Конфиденциальность и обработка данных</h2>
                  <p className="mb-4"><strong>1. Принципы:</strong> «Конфиденциальность по замыслу». Исходный код обрабатывается локально.</p>
                  <p className="mb-4"><strong>2. Собираемые данные:</strong> Информация о подписке, хеш лицензии, логи, технические метаданные. Оплата через третьи стороны.</p>
                  <p className="mb-4"><strong>3. Телеметрия:</strong> Опционально, требует согласия, анонимизируется.</p>
                  <p className="mb-4"><strong>4. Хранение:</strong> Логи (90 дней), Подписка (Срок + 1 год), Платежи (7 лет).</p>
                </section>

                <section id="responsibilities">
                  <h2 className="text-2xl font-semibold mb-4">4. Обязанности и безопасность пользователя</h2>
                  <p className="mb-4"><strong>1. Инфраструктура:</strong> Пользователь обеспечивает безопасность, резервное копирование, обновления, файрвол.</p>
                  <p className="mb-4"><strong>2. Обновления:</strong> Пользователь настраивает SitHub для обновлений.</p>
                  <p className="mb-4"><strong>3. Ответственность:</strong> Провайдер не несёт ответственности за утечки из-за конфигурации/действий пользователя.</p>
                </section>

                <section id="subscription">
                  <h2 className="text-2xl font-semibold mb-4">5. Подписка, оплата и возвраты</h2>
                  <p className="mb-4"><strong>1. Модель:</strong> Годовая подписка.</p>
                  <p className="mb-4"><strong>2. Оплата:</strong> Предоплата, автопродление.</p>
                  <p className="mb-4"><strong>3. Возвраты:</strong> Как правило, возврат не предусмотрен, кроме случаев критического сбоя (&gt;7 дней) или нарушения Провайдером. Для ЕС действует 14-дневный период отказа.</p>
                  <p className="mb-4"><strong>4. Реорганизация:</strong> Передача возможна с уведомлением.</p>
                </section>

                <section id="liability">
                  <h2 className="text-2xl font-semibold mb-4">6. Ответственность, аудит и контакты</h2>
                  <p className="mb-4"><strong>1. Гарантии:</strong> Гарантируется архитектурная невозможность передачи кода. Нет ответственности за косвенный ущерб.</p>
                  <p className="mb-4"><strong>2. Аудит:</strong> Провайдер проводит ежегодные аудиты. Пользователь может проводить аудит собственного трафика.</p>
                  <p className="mb-4"><strong>3. Инциденты:</strong> Уведомление в установленные законом сроки (72ч).</p>
                  <p className="mb-4"><strong>4. Прекращение:</strong> За нарушение/неоплату.</p>
                </section>
